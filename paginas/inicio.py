...
def import_data():
    ...
    import_sheet = get_sheet_from_drive('import')
    ...
    dados['afastamentos'] = get_df_from_drive(import_sheet, 'Atestado') # ["Carimbo de data/hora", "Matrícula SSP", "Primeiro dia de afastamento", 'Último dia de afastamento", "Tipo de afastamento", "Processo SEI"]
    ...
    ## carrega férias   
    dados['ferias'] = get_df_from_drive(import_sheet,'Ferias') # ["Chave", "Carimbo de data/hora", "Matrícula SSP", "Exercício", "1º Período - início", "1º Período - último dia", "2º Período - início", "2º Período - último dia", '3º Período - início", "3º Período - último dia", "SEI"]
    ...
    dados['cargos'] = get_df_from_drive(import_sheet,'Cargos') # ["ID", "CARGO EM COMISSÃO", "Cargo", "Setor", "SIGRH - FUNÇÃO (DEC 46.117)", "Gratificação", "NC_padronizado", "Seq", "Ocupante"]
